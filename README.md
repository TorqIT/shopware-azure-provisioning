...
When adding/updating/removing Container Apps secrets for the PHP-FPM container, you will need to deactivate any active revisions that are using the existing secrets (Azure will throw an error if you do not first deactivate). To deactivate a revision, find its revision number in the Azure Portal, then `exec` into this container and run `./deactivate-php-fpm-container-app-revisions.sh`.
